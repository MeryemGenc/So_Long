...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/26 17:02:00 by mgencali          #+#    #+#             */
/*   Updated: 2023/06/08 15:31:28 by mgencali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	image_create(t_data *data)
{
	...
	data->img.character = mlx_xpm_file_to_image(data->mlx, "img/character.xpm",
			&data->img.img_w, &data->img.img_h);
	data->img.character_right = mlx_xpm_file_to_image(data->mlx,
			"img/character_right.xpm",
			&data->img.img_w, &data->img.img_h);
	data->img.character_left = mlx_xpm_file_to_image(data->mlx,
			"img/character_left.xpm",
			&data->img.img_w, &data->img.img_h);
	data->img.character_back = mlx_xpm_file_to_image(data->mlx,
			"img/character_back.xpm",
			&data->img.img_w, &data->img.img_h);
}
...
static void	data_init(t_data *data)
{
	...
	data->win = mlx_new_window(data->mlx, data->map_width * IMG_SIZE,
			data->map_height * IMG_SIZE, "FeedTheZombie");
	data->coin_count = 0;
